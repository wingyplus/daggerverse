...
defmodule Dagger.Core.QueryBuilder do
  ...
  def execute(selection, client) do
    ...
    case Client.query(client, q) do
      ...
      {:ok, %{status: 200, body: %{"data" => data}}} ->
        {:ok, select_data(data, Selection.path(selection))}
  ...
  defp select_data(data, []), do: data

  defp select_data(data, _selectors) when is_list(data) do
    data
  end

  defp select_data(data, [selector | selectors]) do
    select_data(Map.get(data, selector), selectors)
  end
