...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "Compose":
		...
		case "WithFile":
			...
			return (*Compose).WithFile(&parent, ctx, path)
		default:
	...
	case "":
		return dag.CurrentModule().
			WithObject(
				dag.TypeDef().WithObject("Compose").
					WithFunction(
						dag.NewFunction("WithFile",
							dag.TypeDef().WithObject("Container")).
							WithDescription("WithFile setup service binding from the given compose path.\n").
							WithArg("path", dag.TypeDef().WithKind(Stringkind)))), nil
	default:
