            ...
            .map {
                EnvVariable(
                    QueryBuilder
                        .builder()
                        .select(
                            "loadEnvVariableFromID", args =
                            arrayOf(Arg("id", it["id"]!!))
                        ),
                    engineClient
                )
            }
            ...
            .map {
                FunctionCallArgValue(
                    QueryBuilder
                        .builder()
                        .select(
                            "loadFunctionCallArgValueFromID", args =
                            arrayOf(Arg("id", it["id"]!!))
                        ),
                    engineClient
                )
            ...
            .map {
                Function(
                    QueryBuilder
                        .builder()
                        .select(
                            "loadFunctionFromID", args = arrayOf(
                                Arg(
                                    "id",
                                    it["id"]!!
                                )
            ...
            .map {
                ModuleDependency(
                    QueryBuilder
                        .builder()
                        .select(
                            "loadModuleDependencyFromID", args =
                            arrayOf(Arg("id", it["id"]!!))
                        ),
                    engineClient
                )
            ...
            .map {
                TypeDef(
                    QueryBuilder
                        .builder()
                        .select(
                            "loadTypeDefFromID", args = arrayOf(
                                Arg(
                                    "id",
                                    it["id"]!!
                                )
            ...
            .map {
                ModuleSourceView(
                    QueryBuilder
                        .builder()
                        .select(
                            "loadModuleSourceViewFromID", args =
                            arrayOf(Arg("id", it["id"]!!))
                        ),
                    engineClient
                )
...
public class Client(
    ...
    public fun loadFunctionCallArgValueFromID(id: FunctionCallArgValueID): FunctionCallArgValue {
        var arguments = emptyArray<Arg>()
        arguments += Arg("id", id)
        val newQueryBuilder = queryBuilder.select("loadFunctionCallArgValueFromID", args = arguments)
